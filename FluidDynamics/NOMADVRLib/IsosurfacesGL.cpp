	...
	)
{
	...
	if ((e = glGetError()) != GL_NO_ERROR)
		eprintf("opengl error %d, before glBindVertexArray (IsosurfacesGL), l %d\n", e, __LINE__);
	glBindVertexArray(vao);
	if ((e = glGetError()) != GL_NO_ERROR)
		eprintf("opengl error %d, glBindVertexArray (IsosurfacesGL), l %d\n", e, __LINE__);
	glBindBuffer(GL_ARRAY_BUFFER, vertbuffer);
	if ((e = glGetError()) != GL_NO_ERROR)
		eprintf("opengl error %d, glBindBuffer (IsosurfacesGL), l %d\n", e, __LINE__);
	if (vertdata.size() !=0)
