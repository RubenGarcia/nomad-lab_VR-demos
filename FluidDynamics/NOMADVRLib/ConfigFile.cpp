...
float arrowScalingAcceleration;
float arrowScalingCross;
...
void initState()
{
	...
	isoBufferSize = 20; //default amount of timesteps loaded for isosurfaces
	arrowScalingVelocity = (1.0 / 0.016); //default scaling for arrow size
	arrowScalingAcceleration = 2.0;
	arrowScalingCross = 2.0;
}
...
int loadConfigFile(const char * f)
{
	...
	while (!feof(F)) {
		...
		//added variables move with particles
		//isoBufferSize
		} else if (!strcmp(s, "isobuffersize")) {
			int buffer;
			buffer = 0;
			r = fscanf(F, "%d", &buffer);
			if (buffer != 0)
				isoBufferSize = buffer;
			if (r<1)
				eprintf("Error reading isobuffersize value");
		} else if (!strcmp(s, "velocityscaling")) {
			r = fscanf(F, "%f", &arrowScalingVelocity);
			if (r<1)
				eprintf("Error reading velocityscaling value");
		} else if (!strcmp(s, "accelerationscaling")) {
			r = fscanf(F, "%f", &arrowScalingAcceleration);
			if (r<1)
				eprintf("Error reading accelerationscaling value");
		} else if (!strcmp(s, "crossproductscaling")) {
			r = fscanf(F, "%f", &arrowScalingCross);
			if (r<1)
				eprintf("Error reading crossproductscaling value");
		//particle vector scaling

		//move with particles end
